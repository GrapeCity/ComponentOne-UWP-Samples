...
namespace StockAnalysis.Partial.UserControls
{
    public partial class IntegratedChart
    {
        ...
        private void IndicatorCharts_CollectionChanged(object sender, System.Collections.Specialized.NotifyCollectionChangedEventArgs e)
        {
            ...
            {
                types.Remove(type);
                _indicators.FirstOrDefault(p=>p.Key == type).Value.RemoveAllSeriesFromChart();
        ...
        private void IndicatorsRangeSelectorChanged()
        {
            foreach (var kv in _indicators)
            {
                IEnumerable<Object.QuoteRange> ranges = kv.Value.GetYRange(Model.LowerValue == null ? 0 : Model.LowerValue.Value, Model.UpperValue.Value);
        ...
        private void CreatePlotArea(int rowCount)
        {
            ...
            financialChart.PlotAreas.Add(new C1.Xaml.Chart.PlotArea() { PlotAreaName = "plot0", Row = 0});
            int rowIndex = 1;
            for (int i = 1; i < rowCount; i++)
            {
                financialChart.PlotAreas.Add(new C1.Xaml.Chart.PlotArea() { PlotAreaName = "spacing" + i, Row = rowIndex++, Height = new Windows.UI.Xaml.GridLength( i == 1 ? 15 : 5) });
                financialChart.PlotAreas.Add(new C1.Xaml.Chart.PlotArea() { PlotAreaName = "plot" + i, Row = rowIndex++, Height = new Windows.UI.Xaml.GridLength(30) });
        ...
        {
            return Activator.CreateInstance(Type.GetType(type.FullName), new object[] { this.financialChart, plotAreaName }) as IIndicator;  
        }
