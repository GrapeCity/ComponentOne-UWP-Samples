...
Public Class AnnotationViewModel
    ...
    Private _simpleData As List(Of DataItem)
    ...
    Public ReadOnly Property SimpleData() As List(Of DataItem)
        Get
            If _data Is Nothing Then
                _data = New List(Of DataItem)()
                _data.Add(New DataItem() With {.X = 1, .Y = 30})
                _data.Add(New DataItem() With {.X = 2, .Y = 20})
                _data.Add(New DataItem() With {.X = 3, .Y = 30})
                _data.Add(New DataItem() With {.X = 4, .Y = 65})
                _data.Add(New DataItem() With {.X = 5, .Y = 70})
                _data.Add(New DataItem() With {.X = 6, .Y = 60})
            End If
